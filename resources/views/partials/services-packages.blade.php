{{-- Expects: $packagesTitle (string), $packages (array of ['name','tagline','price','priceNote'=>optional,'bullets'=>[], 'featured'=>bool]) --}}
<h2 class="font-display text-2xl font-bold mb-2" style="color: var(--ink);">{{ $packagesTitle }}</h2>
<p class="text-sm mb-6" style="color: var(--ink-faint);">Starting prices in NPR — final cost depends on your exact requirements. <a href="{{ route('contact') }}" class="font-semibold hover:underline" style="color: var(--accent);">Contact me</a> for a precise quote.</p>
<div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
    ...
    <div class="glass-card p-6" style="{{ !empty($pkg['featured']) ? 'border-color: var(--accent);' : '' }}">
        ...
        <p class="text-xs mb-3" style="color: var(--ink-faint);">{{ $pkg['tagline'] }}</p>
        @if(!empty($pkg['price']))
        <div class="mb-4 pb-4" style="border-bottom: 1px solid var(--line);">
            <span class="font-display text-2xl font-extrabold" style="color: var(--accent);">{{ $pkg['price'] }}</span>
            @if(!empty($pkg['priceNote']))
            <span class="block text-xs mt-0.5" style="color: var(--ink-faint);">{{ $pkg['priceNote'] }}</span>
            @endif
        </div>
        @endif
